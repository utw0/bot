...
client.on("guildMemberAdd", async member => {
  ...
  let inviter = usedInvite && usedInvite.inviter && usedInvite.inviter.id ? await client.users.fetch(usedInvite.inviter.id) : member.guild.id;
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) global.send(inviteChannel, `${client.emojis.cache.find(x => x.name === "supheli")} ${member} (\`${member.id}\`) __Kullanıcısıyla beraber__ \`${member.guild.memberCount}\` __Kişi Olduk.__\n\n${client.emojis.cache.find(x => x.name === "supheli")} __Davet Eden:__ ${inviter === member.guild.id ? member.guild.name : inviter}\n\n${client.emojis.cache.find(x => x.name === "supheli")} Ulaştığı Davet Sayısı:\`${(x.regular ? x.regular : 0) + (x.bonus ? x.bonus : 0)}\`\n${client.emojis.cache.find(x => x.name === "supheli")} __Durum:__ \`Şüpheli⛔\`\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬`).catch(err => { });
          member.roles.set([ayarlar.fakeHesapRolu]);
           global.send(member.guild.channels.cache.find(c => c.name === ayarlar.teyitKanali),(`${member} üyesi sunucuya katıldı fakat hesabı ${member.client.tarihHesapla(member.user.createdAt)} açıldığı için ${client.guilds.cache.get(ayarlar.sunucuID).roles.cache.get(ayarlar.fakeHesapRolu).toString()} rolü verildi!`));
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(async x => {
          ...
          if (inviteChannel) global.send(inviteChannel, `${client.emojis.cache.find(x => x.name === "hosgeldinKonfeti")} ${member} (\`${member.id}\`) __Kullanıcısıyla beraber__ \`${member.guild.memberCount}\` __Kişi Olduk.__\n\n${client.emojis.cache.find(x => x.name === "hosgeldinKonfeti")} __Davet Eden:__ ${inviter === member.guild.id ? member.guild.name : inviter}\n\n${client.emojis.cache.find(x => x.name === "hosgeldinKonfeti")} __Ulaştığı Davet Sayısı:__\`${(x.regular ? x.regular : 0) + (x.bonus ? x.bonus : 0)}\`\n\n${client.emojis.cache.find(x => x.name === "hosgeldinKonfeti")} __Durum:__ \`Başarılı☑️\`\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬`).catch(err => { });
          ...
          if (cezalar.filter(d => (!d.bitisTarihi || d.bitisTarihi > Date.now()) && (d.cezaTuru === "JAIL" || d.cezaTuru === "REKLAM" ||d.cezaTuru === "TEMP-JAIL")).length) {
            ...
            global.send(member.guild.channels.cache.find(x => x.name === ayarlar.teyitKanali).send(`          
:tada: Sunucumuza hoş geldin ${member} - (\`${member.id}\`)
\nHesabın **${member.user.createdAt.toTurkishFormatDate()}** tarihinde (\`${member.client.tarihHesapla(member.user.createdAt)}\`) oluşturulmuştur.)
\nSunucu kurallarımız ${client.channels.cache.find(x => x.name === "rules")} kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.
\n${inviter === member.guild.id ? member.guild.name : inviter} Toplam **${(x.regular ? x.regular : 0) + (x.bonus ? x.bonus : 0)}** davetini gerçekleştirerek sunucumuzun üyesi olmanı sağladı! İyi eğlenceler **${member.guild.memberCount}** Kişi olduk :tada:`)).catch(console.error);
...
\nHesabın **${member.user.createdAt.toTurkishFormatDate()}** tarihinde (\`${member.client.tarihHesapla(member.user.createdAt)}\`) oluşturulmuştur.) 
\nSunucu kurallarımız ${client.channels.cache.find(x => x.name === "rules")} kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.
\n${inviter === member.guild.id ? member.guild.name : inviter} Toplam **${(x.regular ? x.regular : 0) + (x.bonus ? x.bonus : 0)}** davetini gerçekleştirerek sunucumuzun üyesi olmanı sağladı! İyi eğlenceler 00${member.guild.memberCount}** Kişi olduk :tada:`)).catch(console.error);
                      }
        });
      };
    });
    ...
    if (memberData && memberData.inviterID) {
      ...
      Database.findOne({ guildID: member.guild.id, userID: memberData.inviterID }, async (err, inviterData) => {
        if (!inviterData) {
          ...
          inviterData.save().then(x => {
            if (inviteChannel) global.send(inviteChannel, `${client.emojis.cache.find(x => x.name === "quit")} ${member} (\`${member.id}\`) __Belirtilen Üye Sunucudan Ayrıldı__ \`${member.guild.memberCount}\` __Kişi Kaldık__ ${client.emojis.cache.find(x => x.name === "ck")}\n\n${client.emojis.cache.find(x => x.name === "quit")}__Davet Bilgisi:__ ${inviter === member.guild.id ? member.guild.name : inviter}\n\n${client.emojis.cache.find(x => x.name === "quit")}__Kalan Davet Sayısı:__ \`${(x.regular ? x.regular : 0) + (x.bonus ? x.bonus : 0)}\`\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬`).catch(err => { });
          });
        };
      });
    } else {
      if (inviteChannel) global.send(inviteChannel, `${client.emojis.cache.find(x => x.name === "quit")} ${member.user.tag} (\`${member.id}\`) __Belirtilen Üye Sunucudan Ayrıldı__ \`${member.guild.memberCount}\` __Kişi Kaldık__ ${client.emojis.cache.find(x => x.name === "ck")}\n\n${client.emojis.cache.find(x => x.name === "quit")}__Davet Bilgisi:__ \`Davetçi bulunamadı!\`\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬`).catch(err => { });
...
client.on("message", async message => {
  ...
  if (command === "hedefdaveti" || command === "hedefdavet" || command === "hedef-davet") {
...
;
    ...
    message.channel.send(embed.setDescription(`Toplam **${haftalikDavetVerisi.regular ? haftalikDavetVerisi.regular.length : 0}** davete sahip! (**${haftalikDavetVerisi.leaves ? haftalikDavetVerisi.leaves.length : 0}** ayrılan)`));
  };
...
;
  ...
  if (command === 'rollog' || command === 'rol-log') {
    ...
    let page = 1;
    const msg = await message.channel.send(embed.setAuthor(user.tag, user.avatarURL({ dynamic: true })).setDescription(`${liste.slice(page == 1 ? 0 : page * 9 - 9, page * 9).join("\n")}`));
  ...
  if (command === "alarım" || command === "alarm") {
    ...
    if (!sure) return global.send(message.channel, "Geçerli bir süre belirtmelisin! (1s/1m/1h/1d)").then(x => x.delete({ timeout: 5000 }));
    if (!args[1]) return global.send(message.channel, "Hatırlatmamı istediğin şeyi belirtmelisin!").then(x => x.delete({ timeout: 5000 }));
    ...
    yeniAlarm.save();
    global.send(message.channel, "Alarım Kuruldu.").then(x => x.delete({ timeout: 10000 }));
  };
  ...
  if (command === "haftalık-sıfırla" || command === "haftalıksıfırla") {
    ...
    message.reply("Haftalık davetler başarıyla sıfırlandı!");
  };
  ...
  if (command === "bonus") {
    ...
    let sayi = args[1];
    if (!uye || !sayi) return global.send(message.channel, `Geçerli bir üye ve sayı belirtmelisin! (${ayarlar.inviteprefix}bonus @üye +10/-10)`);
    Database.findOne({ guildID: message.guild.id, userID: uye.id }, (err, inviterData) => {
      if (!inviterData) {
        let newInviter = new Database({
          _id: new mongoose.Types.ObjectId(),
        ...
        newInviter.save().then(x => global.send(message.channel, `Belirtilen üyenin bonus daveti **${sayi}** olarak ayarlandı!`));
      } else {
        eval(`inviterData.bonus = inviterData.bonus+${Number(sayi)}`);
        inviterData.save().then(x => global.send(message.channel, `Belirtilen üyenin bonus davetine **${sayi}** eklendi!`));
      };
    });
  };
...
client.on("message", (message) => {
  if (message.activity && message.activity.type === 3 && !message.deleted) {
    if (!client.kullanabilir(message.author.id) && !ayarlar.commandkanali.includes(message.channel.name)) return message.delete(), global.send(message.channel, `${message.author}, Bu sunucuda chate spotify linki atmak yasaktır.`).then(x => x.delete({ timeout: 7500 }))
  }
});
client.on("message", (message) => {
  if(message.channel.name === "spotify-paylaşım" && message.author.bot && client.kullanabilir(message.author.id)) return;
  if (message.activity && message.activity.type !== 3 && !message.deleted) {
      message.delete() 
      global.send(message.channel, `${message.author}, Bu kanalda spotify paylaşımı dışında birşey yapılması yasaktır!`).then(x => x.delete({ timeout: 7500 }))
  }
});
