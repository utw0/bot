...
if((message.guild.ownerID != message.author.id)) return global.send(message.channel, '**Bunu yapmak için yeterli yetkin yok!**');
let secim = args[0];
...
if (secim === 'sıfırla') {
if (!args[1] || (args[1] !== 'özel' && args[1] !== 'ses' && args[1] !== 'chat' && args[1] !== 'teyit')) return global.send(message.channel, embed.setDescription('Sıfırlanacak veriyi belirtmelisin! (ses/chat/teyit)')).then(x => x.delete({timeout: 5000}));
if (args[1] === 'ses') {
...
return global.send(message.channel, embed.setDescription('Başarıyla belirtilen istatistik verileri sıfırlandı!'));
}
if (!secim) return global.send(message.channel, embed.setDescription('Ses, chat veya teyit istatistiklerini sıfırlamak istiyorsan **sıfırla ses/chat/teyit** bir ayar yapmak istiyorsan aşağıdaki seçimleri kullanmalısın.'));
};
