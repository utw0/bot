...
module.exports.execute = async(client, message, args, ayar) => {
    if(!client.kullanabilir(message.author.id)) return global.reply(message, 'bu komutu kullanabilmek için gerekli rollere sahip değilsin!').then(x => x.delete({timeout: 5000}));
    ...
    if (!['haftalık', 'haftalik', 'genel', 'tüm'].includes(args[0])) return global.reply(message, 'Hangi verilere bakacağını belirtmelisin! (haftalık/genel)');
    if (!rol) return global.reply(message, 'geçerli bir rol belirtmelisin!').then(x => x.delete(5000));
    ...
    setTimeout(() => {
        mesaj.delete();
        global.send(message.channel, table(sonuc), {split: true, code: 'xl'});
    }, 5000);
};
