...
module.exports.execute = async(client, message, args,ayar) => {
   ...
   let kullanici = message.mentions.users.first() || client.users.cache.get(args[0]) || (args.length > 0 ? client.users.cache.filter(e => e.username.toLowerCase().includes(args.join(' ').toLowerCase())).first(): message.author) || message.author;
    ...
    MemberStats.findOne({ guildID: message.guild.id, userID: uye.id }, (err, data) => {
        ...
        embed.addField('Haftalık Ses',`${client.emojis.cache.find(x => x.name === "mav")}\`Toplam:\`  ** ${client.convertDuration(haftalikSesToplam)}** \n ${haftalikSesListe}`);
        embed.addField('Haftalık Chat',`${client.emojis.cache.find(x => x.name === "mav")}\`Toplam:\`  ** ${haftalikChatToplam} mesaj** \n ${haftalikChatListe}\n${yetkiliBilgisi}`);
