...
client.kullanabilir = function (id) {
  if (client.guilds.cache.get(sunucuAyar.sunucuID).members.cache.get(id).hasPermission("ADMINISTRATOR") || client.guilds.cache.get(sunucuAyar.sunucuID).members.cache.get(id).hasPermission("MANAGE_ROLES")) return true;
  return false;
};
...
client.on("message", (message) => {
  if (message.content.toLowerCase().startsWith('!tag') || message.content.toLowerCase().startsWith('.tag') || message.content.toLowerCase().startsWith('?tag')) {
    ...
    message.channel.send(sunucuAyar.davet);
