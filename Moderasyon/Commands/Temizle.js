...
module.exports.execute = async (client, message, args) => {
    ...
    if (!message.member.hasPermission('MANAGE_MESSAGES')) return global.send(message.channel, embed.setDescription('Bu komutu kullanabilmek için **Mesajları Yönet** iznine sahip olmalısın!')).then(x => x.delete({ timeout: 5000 }));
    ...
    message.channel.bulkDelete(Number(args[0])).then(msjlar => global.send(message.channel, embed.setDescription(`Başarıyla **${msjlar.size}** adet mesaj silindi!`)).then(x => x.delete({ timeout: 5000 }))).catch(() => {
        return undefined;
    });
};
