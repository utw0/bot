...
module.exports.execute = async (client, message, args, conf, emoji) => {
	...
	if (banLimitleri.get(message.author.id) >= 15) return global.reply(message, `\`${this.configuration.name} komutu için limite ulaştın!\``).then(x => x.delete({ timeout: 5000 }))
	if (message.member.roles.highest.position <= member.roles.highest.position) return global.send(message.channel, embed.setDescription('Belirttiğin kişi senden üstün veya onunla aynı yetkidesin!')).then(x => x.delete({ timeout: 5000 }));
	...
	global.send(message.channel, embed.setDescription(`${member.toString()} kişisinin ismi "${displayName.slice(2)}" olarak değiştirildi.`)).then((x) => x.delete({ timeout: 10000 }));
