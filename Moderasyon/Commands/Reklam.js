...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    if (!client.kullanabilir(message.author.id) && !ayar.reklamciRolu.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "Iptal")}`);

    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if (banLimitleri.get(message.author.id) >= ayar.reklam) return global.reply(message, `\`${this.configuration.name} komutu için limite ulaştın!\``).then(x => x.delete({ timeout: 10000 }));
    ...
    if (uye.roles.highest.position >= message.guild.roles.cache.get(ayar.enAltYetkiliRolu).position) return global.send(message.channel, '`Yetkililer birbirlerini jaile atamazlar.`').then(x => x.delete({ timeout: 5000 }));
    ...
    global.send(message.channel, embed.setDescription(`${uye} isimli kullanıcıya ${message.author} tarafından ${message.guild.roles.cache.get(ayar.reklamRolu).toString()} rolü verildi!`)).then(x => x.delete({ timeout: 15000 })).catch();
    global.send(client.channels.cache.find(c => c.name === ayar.reklamLogKanali), new MessageEmbed().setColor('BLUE').setDescription(`Engelleyen: ${message.author} \`(${message.author.id})\`\nEngellenen: ${uye.user.tag} \`(${uye.id})\`\nCeza-i İşlem: Reklam \`(${cezaNumara})\``)).catch(console.error);
    message.react("☑️")
