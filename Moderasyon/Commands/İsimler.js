...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    if (!memberData.history) return global.send(message.channel, embed.setAuthor(member.displayName, kullanici.avatarURL({ dynamic: true })).setDescription('Üyenin herhangi bir kayıtı bulunamadı.'));
    global.send(message.channel, embed.setAuthor(member.displayName, kullanici.avatarURL({ dynamic: true })).setDescription([
