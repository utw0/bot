...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    if (banLimitleri.get(message.author.id) >= 5) return global.reply(message, `\`${this.configuration.name} komutu için limite ulaştın!\``);
    ...
    if (!reason) return global.send(message.channel, embed.setDescription('**Lütfen geçerli bir uyarı sebebi gir!**')).then(x => x.delete({ timeout: 5000 }));
    ...
    collector.on('collect', async (reaction) => {
        if (reaction.emoji.name == "1️⃣") {
            ...
            uye.send(`${reason} sebebiyle ${message.author} adlı yetkili tarafından ilk uyarınızı aldınız. Uyarı almaya devam etmeniz taktirinde yetkileriniz alınacak veya düşürülecektir. 14 gün sonunda herhangi bir uyarı almaz iseniz uyarı perminiz üzerinzden alınır ve siciliniz temizlenir.`)
            global.send(message.channel, embed.setDescription(`${uye} \`(${moment.duration(ms(sure)).format('D [gün,] H [saat,] m [dakika]')})\` boyunca ${reason} sebebiyle <@${ayar.uyarı1}> rolü verildi!`)).catch(console.error);
            global.send(client.channels.cache.get(ayar.uyarilog), new MessageEmbed().setColor('f39c12').setDescription(`Uyaran: ${message.author} \`(${message.author.id})\`\nUyarılan: ${uye.user.tag} \`(${uye.id})\`\nSebep: ${reason}\nCeza-i İşlem: Uyarı(1) \`(${cezaNumara})\``)).catch(console.error);

            newPenalty.save();
        } else if (reaction.emoji.name == "2️⃣") {

            if (!uye.roles.cache.get(ayar.uyarı1)) return global.send(message.channel, new MessageEmbed().setDescription("`Üyeye uyarı 2 verebilmek için önce uyarı 1 alması gerekmektedir.`")).then(x => x.delete({ timeout: 10000 }));
            uye.roles.add(ayar.uyarı2).catch(console.error);
            global.send(message.channel, embed.setDescription(`${uye} \`(${moment.duration(ms(sure)).format('D [gün,] H [saat,] m [dakika]')})\` boyunca ${reason} sebebiyle <@${ayar.uyarı2}> rolü verildi!`)).catch(console.error);
            global.send(client.channels.cache.get(ayar.uyarilog), new MessageEmbed().setColor('f39c12').setDescription(`Uyaran: ${message.author} \`(${message.author.id})\`\nUyarılan: ${uye.user.tag} \`(${uye.id})\`\nSebep: ${reason}\nCeza-i İşlem: Uyarı(1) \`(${cezaNumara})\``)).catch(console.error);
         let newPenalty = new Penalty({
                sunucuID: message.guild.id,
                uyeID: uye.id,
            ...
            if (!client.kullanabilir(message.author.id) && !ayar.jailciRolleri.some(rol => message.member.roles.cache.has(rol))) return global.send(message.channel, embed.setDescription('`Yeterli Yetkin Bulunmamakta.`')).then(x => x.delete({ timeout: 5000 }));
            if (!uye.roles.cache.get(ayar.uyarı2)) return global.send(message.channel, new MessageEmbed().setDescription("`Üyeye uyarı 3 verebilmek için önce uyarı 2 alması gerekmektedir.`")).then(x => x.delete({ timeout: 10000 }));
            ...
            global.send(message.channel, embed.setDescription(`${uye} \`(${moment.duration(ms(sure)).format('D [gün,] H [saat,] m [dakika]')})\` boyunca ${reason} sebebiyle uyarı limitini geçtiği için jaile atıldı!`)).catch(console.error);
            global.send(client.channels.cache.get(ayar.uyarilog), new MessageEmbed().setColor('c0392b').setDescription(`Uyaran: ${message.author} \`(${message.author.id})\`\nUyarılan: ${uye.user.tag} \`(${uye.id})\`\nSebep: ${reason}\nCeza-i İşlem: Uyarı(3)  \`(${cezaNumara})\``)).catch(console.error);
            let newPenalty = new Penalty({
                sunucuID: message.guild.id,
                uyeID: uye.id,
                yetkiliID: message.author.id,
            ...
            global.send(client.channels.cache.get(ayar.uyarilog), new MessageEmbed().setColor('GREEN').setDescription(`Engeli Kaldıran: ${message.author} \`(${message.author.id})\`\nEngeli Kalkan: ${uye} \`(${uye.user.tag})\`\nCeza-i İşlem: Uyarı`)).catch(console.error);
            Penalty.find({ sunucuID: message.guild.id, uyeID: uye.id }).sort({ atilmaTarihi: -1 }).exec(async (err, data) => {
                ...
                if (!cezalar.length) return message.reply("Bu kullanıcı uyarı almamış.")
    ...
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);Iptal
