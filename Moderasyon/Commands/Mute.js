...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    if (!client.kullanabilir(message.author.id) && !ayar.sesMuteciRolleri.some(rol => message.member.roles.cache.has(rol))  && !ayar.muteciRolleri.some(rol => message.member.roles.cache.has(rol))) return global.send(message.channel, embed.setDescription('`Yeterli Yetkin Bulunmamakta.`')).then(x => x.delete({ timeout: 5000 }));
    ...
    collector.on('collect', async (reaction) => {
        if (reaction.emoji.name == "chatmute") {
            ...
            if (!client.kullanabilir(message.author.id) && !ayar.muteciRolleri.some(rol => message.member.roles.cache.has(rol))) return global.send(message.channel, embed.setDescription('`Ne yazık ki chat mute atabilecek yetkiye sahip değilsin ancak ses mute atabilirisin`')).then(x => x.delete({ timeout: 10000 }));
            ...
            global.send(message.channel, embed.setDescription(`${uye} \`(${moment.duration(ms(sure)).format('D [gün,] H [saat,] m [dakika]')})\` dakika boyunca ${reason} sebebiyle chat kanallarında susturuldu!`)).catch(console.error);
            global.send(client.channels.cache.find(c => c.name === ayar.muteLogKanali), new MessageEmbed().setColor('f39c12').setDescription(`Engelleyen : ${message.author} \`(${message.author.id})\`\nEngellenen: ${uye} \`(${uye.id})\`\nSebep: ${reason}\nSüre: ${new Date(bitisTarihi).toTurkishFormatDate()} \`(${moment.duration(ms(sure)).format('D [gün,] H [saat,] m [dakika]')})\`\nCeza-i İşlem: Chat Mute \`(${cezaNumara})\``)).catch(console.error);
            ...
            global.send(message.channel, embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${moment.duration(ms(sure)).format('D [gün,] H [saat,] m [dakika]')}** boyunca **${reason}** nedeniyle seste mutelendi!`)).catch();
            global.send(client.channels.cache.find(c => c.name === ayar.sesMuteLogKanali), new MessageEmbed().setColor('f39c12').setDescription(`Engelleyen : ${message.author} \`(${message.author.id})\`\nEngellenen: ${uye} \`(${uye.id})\`\nSebep: ${reason}\nSüre: ${new Date(bitisTarihi).toTurkishFormatDate()} \`(${moment.duration(ms(sure)).format('D [gün,] H [saat,] m [dakika]')})\`\nCeza-i İşlem: Ses Mute \`(${cezaNumara})\``)).catch(console.error);
