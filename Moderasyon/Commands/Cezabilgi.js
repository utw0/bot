...
module.exports.execute = async (client, message, args, ayar) => {
    ...
    const ID = args[0];
    if (!ID || isNaN(ID)) return global.send(message.channel, cezaBilgiEmbed);
    Penalty.find({ sunucuID: message.guild.id }).exec(async (err, cezalar) => {
        ...
        if (ceza) {
            ...
            global.send(message.channel, cezaBilgiEmbed).then(x => x.delete({ timeout: 15000 })).catch(console.error);
        }
    });
    message.react("☑️")
