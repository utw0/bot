...
global.conf = conf;
const yasaklitag = global.yasaklitag = require("../yasaklitag.json");
...
\`\`\`Kullanıcı: ${newMember.user.tag} (${newMember.user.id})\nDeğişim Tarihi: ${moment(Date.now()).locale("tr").format("LLL")}\`\`\``)
  ...
  client.channels.cache.find(c => c.name === conf.nickname_log_basit).send(`:file_folder: ${newMember.user.tag} (\`${newMember.user.id}\`) üyesinin sunucu içi ismi değişti \`${oldMember.displayName}\` **>** \`${newMember.displayName}\``).catch(() => {});
});
...
client.login(sunucuAyar.LevelBotToken).then(x => console.log(`Bot ${client.user.tag} olarak giriş yaptı!`)).catch(err => console.error(`Bot giriş yapamadı | Hata: ${err}`));